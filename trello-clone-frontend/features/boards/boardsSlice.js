...
export const fetchBoardDetails = createAsyncThunk(
  ...
  async (boardId, { rejectWithValue }) => {
    try {
      ...
      console.log('[fetchBoardDetails thunk] Raw API response data:', JSON.stringify(response.data, null, 2));
      return response.data.data;
    } catch (error) {
...
export const addCardToList = createAsyncThunk(
  ...
  async ({ listId, title, description, dueDate, assignedUserID, supervisorID, color }, { rejectWithValue }) => { // Added color
    const payload = { title, description, dueDate, assignedUserID, supervisorID, color }; // Added color
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchBoardDetails.fulfilled, (state, action) => {
        console.log('[fetchBoardDetails.fulfilled] Raw action.payload:', JSON.stringify(action.payload, null, 2));
        ...
        console.log('[fetchBoardDetails.fulfilled] state.currentBoard after update:', JSON.stringify(state.currentBoard, null, 2));
      })
      .addCase(fetchBoardDetails.rejected, (state, action) => {
        console.error('[fetchBoardDetails.rejected] Error payload:', JSON.stringify(action.payload, null, 2));
...
// All action creators are automatically generated by createSlice and included in boardsSlice.actions
export const {
    ...
    cardDeletedByWS,
    cardMovedByWS
    // No need to list webSocketActions separately if they are part of boardsSlice.actions
} = boardsSlice.actions;

// If components need to import all WS actions under a specific namespace, this is one way:
export const webSocketActions = {
    boardUpdatedByWS: boardsSlice.actions.boardUpdatedByWS,
    boardDeletedByWS: boardsSlice.actions.boardDeletedByWS,
    listCreatedByWS: boardsSlice.actions.listCreatedByWS,
    listUpdatedByWS: boardsSlice.actions.listUpdatedByWS,
    listDeletedByWS: boardsSlice.actions.listDeletedByWS,
    cardCreatedByWS: boardsSlice.actions.cardCreatedByWS,
    cardUpdatedByWS: boardsSlice.actions.cardUpdatedByWS,
    cardDeletedByWS: boardsSlice.actions.cardDeletedByWS,
    cardMovedByWS: boardsSlice.actions.cardMovedByWS,
};
